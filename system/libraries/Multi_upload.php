...
//            echo "$fname\n\n<br><br>\n\n";
			...
			$rname=time()."_img_".$CI->upload->file_name;
            if ( ! @copy($CI->upload->file_temp, $CI->upload->upload_path.$rname))
            {
				
                if ( ! @move_uploaded_file($CI->upload->file_temp, $CI->upload->upload_path.$rname))
                {
                      $error_hold[$i] = 'upload_destination_error';
                }
				 
            }
			$sel="insert into gallery_image(gellery_id,image) values('".$_REQUEST['gid']."','".$rname."')";
		    $res=mysql_query($sel);
			header("location:".base_url()."index.php/gallery/gallerydetail");
			//redirect(base_url() . 'index.php/gallery/gallerydetail');
